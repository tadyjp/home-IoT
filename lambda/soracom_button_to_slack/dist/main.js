...
var Notification = class {
  ...
  async execute(event) {
    if (event.imei !== this.mailboxButtonIMEI) {
      throw new Error(`IMEI mismatch: ${event.imei} is not ${this.mailboxButtonIMEI}`);
    }
...
function parseEvents(records) {
  ...
  for (const record of records) {
    ...
    if (body.imei === mailboxButtonIMEI) {
      list.push(new MessageEvent(body.imei, new EventRaw(body.payloads.clickType, body.payloads.clickTypeName, body.payloads.batteryLevel, body.payloads.binaryParserEnabled)));
    }
  }
  return list;
}
